...
//Because foo is declaired in the scope of for statement
...
//Because x is declaired in the scope of main
...
func greeting(greeting: String?, name: String) {
    if let greetText = greeting {
        print("\(greetText), \(name).")
    } else {
        print("Hello, \(name).")
    }
}

greeting(greeting: "Hello there", name: "Dima")
greeting(greeting: nil, name: "Alice")
